...
class Signal
{
	...
	public function construct() {
		...
		$this->set('credential.type', 'html');

		$this->set('time', $this->getTimeInSeconds());
		$this->set('include', []);

		$this->set('request.uri', $_SERVER['REQUEST_URI']);
		$this->set('request.docs', $_SERVER['DOCUMENT_ROOT']);
		$this->set('request.scheme', $_SERVER['REQUEST_SCHEME']);
		$this->set('request.query', $_SERVER['QUERY_STRING']);
		$this->set('request.method', $_SERVER['REQUEST_METHOD']);
		$this->set('server.gateway', $_SERVER['GATEWAY_INTERFACE']);
		$this->set('server.server_ip', $_SERVER['SERVER_ADDR']);
		$this->set('server.remote_ip', $_SERVER['REMOTE_ADDR']);
		$this->set('server.server_port', $_SERVER['SERVER_PORT']);
		$this->set('server.server_protocol', $_SERVER['SERVER_PROTOCOL']);
		$this->set('server.server_software', $_SERVER['SERVER_SOFTWARE']);
		$this->set('http.host', $_SERVER['HTTP_HOST']);
		$this->set('http.agent', $_SERVER['HTTP_USER_AGENT']);
		$this->set('http.accept', $_SERVER['HTTP_ACCEPT']);
		$this->set('http.connection', $_SERVER['HTTP_CONNECTION']);
		$this->set('http.accept_encoding', $_SERVER['HTTP_ACCEPT_ENCODING']);
		$this->set('http.accept_languange', $_SERVER['HTTP_ACCEPT_LANGUAGE']);
		$this->set('request.get', $_GET);
		$this->set('request.post', $_POST);
		$this->set('request.files', $_FILES);
		$this->set('request.cookies', $_COOKIE);
		...
		if($this->include_exists('_common/config/settings')) {
			self::$memory = $this->merge(self::$memory, $this->include('_common/config/settings'));
			$this->conclude();
		}

	}

	public function cpu_setCredential($cred) {
		$this->set('credential.type', $cred);
	}

	public function cpu_setDoctype($type) {
		if($this->isset('doctype')) {
			$this->fail('Doctype allready seted.');
		} else {
	...
	public function username($unme) {
		$this->set('user.username', $unme);
	}

	public function password($upsw) {
		$this->set('user.password', $upsw);
	}

	public function setDocumentRootDirective($disk) {
		$this->set('disk.local', $disk);
	}

	public function conclude() {
		$this->request();
		$controller_name = $this->get('request.controller');
		$method_name = $this->get('request.method');

		$controller_name = '\App\Controllers\\'.$this->ucfirst($controller_name);

		if(class_exists($controller_name)) {
			$controller = new $controller_name;
			if(method_exists($controller, $method_name)) {
				$data = $controller->{$method_name}($this);
			} else {
	...
	public function request() {
		...
		$this->set('request.controller', 'desktop');
		$this->set('request.method', 'index');
		$this->set('request.dynamic_params', []);

		if($this->get('request.uri') != $this->fslash()) {
			$uri = $this->explode($this->fslash(), $this->get('request.uri'));
			if(isset($uri[0])) {
				if($uri[0] == 'desktop' && !isset($uri[1])) {
					$this->fail('404: Page not found.');
				} else {
					$this->set('request.controller', $uri[0]);
					unset($uri[0]);
				}
				if(isset($uri[1])) {
					if($uri[1] == 'index') {
						...
						$this->set('request.method', $uri[1]);
						unset($uri[1]);
					}
					if(isset($uri[2])) {
						$this->set('request.uri_params', $this->is_empty($uri) ? false : $this->array_zero($uri));
					}
				}
			}
		}
	...
	public function chkeep() {
		if(!$this->is_empty($this->get('keep'))) {
			if($this->isset('keep.attr')) {
				if($this->get('keep.attr.name') !== '') {
					if($this->is_same('keep.attr.level', 'this.level')) {
						$this->unset('keep.attr');
					}
				}
			}
			if($this->isset('keep.attr_nested')) {
				if($this->is_same('keep.attr_nested.level', 'this.level')) {
					$this->unset('keep.attr_nested');
				}
			}
			if($this->isset('keep.form')) {
				if($this->is_same('keep.form.level', 'this.level')) {
					$this->unset('keep.form');
				}
			}
		}
	}
	...
	public function tag($tag = null) {
		...
		if(!$tag) {
			...
			if($this->isset_and_true('settings.unclosed_tags')) {
			...
			$this->set('tag.credential', $this->get('credential.type'));
		}
		return $this;
	}
	...
	public function getDateTimeZone($zone) {
		return new DateTimeZone($zone);
	}

	public function mytime() {
		$datetime = new DateTime("now", $this->getDateTimeZone('Europe/Athens'));
		$datetime->setTimestamp($this->getTimeInSeconds());
		$time = $datetime->format('Y/d/m-H:i:s');

		$time = $this->explode('-', $datetime->format('Y/d/m-H:i:s'));

		$date = $time[0];
		$date = $this->explode('/', $date);
		...
		$time = $time[1];
		$time = $this->explode(':', $time);

		$our = $time[0];
		$minutes = $time[1];
		$seconds = $time[2];

		return $day.' '.$this->getMonth($month).' '.$year.' - '.$our.':'.$minutes.':'.$seconds;
	}
	...
	public function monitor() {
		$this->setView($this->to_bslash($this->get('request.controller').'/'.$this->get('request.method')));
		...
		if($this->isset_and_true('monitor.info')) {
			$this->text($this->get('monitor.info'));
		}
		if($this->isset_and_true('monitor.doctype')) {
			if($this->get('monitor.doctype', 'html')) {
				$this->tag('doctype')->attr('html')->tag();
				$this->tag('html');
					if($this->isset_and_true('monitor.doctype_lang')) {
					...
					$this->tag();
					$this->tag('body');
						$this->tag('div')->attr('style', 'margin:0 auto;background-color:#dddddd;border:1px solid #222222;width:'.$this->get('monitor.display.current.width').'px;height:'.$this->get('monitor.display.current.height').'px;');
	...
	public function getExportedString($list = []) {
	...
	public function unique_id($str = '', $id = null, $length = 5) {
			...
			for($i=0;$i<count($crypt);$i++) {
				...
				if(in_array($id, $this->get('unique_ids'))) {
					return $this->unique_id($str, $id, $length);
				}

				$this->push('unique_ids', $id);
	...
	public function get_include_contents($file) {
			return $this->getFileContents($this->project($file.'.php'));
		}
			...
			if($this->get('this.has_open_tag')) {
				if($this->credential()) {
					...
					$this->set('this.document.'.$this->get('this.index'), [
					    'credential'				=> $this->get('tag.credential'),
						'tag'						=> $this->get('tag.tag'),
					    'attr'						=> $this->get('tag.attr'),
					    'attr_nested'				=> $this->get('tag.attr_nested'),
					...
					$this->fail('Security error: Credentials has fail to process your request.');
				}
			}
		}
	...
	public function credential() {
			...
			if(!$this->isset('credential.type')) {
				$this->fail('Credential is missing.');
			}

			if($this->get('credential.type', 'html')) {
				if($this->get('tag.tag') !== 'untaged') {
					...
					if($this->isset('tag.attr.name')) {
						$this->set('keep.attr', [
							'level' => $this->get('this.level'),
							'name' => $this->replace('[]', '', $this->get('tag.attr.name')),
							'type' => $this->isset('tag.attr.type') ? $this->get('tag.attr.type') : false,
							'defineds' => $this->get('tag.defineds')
						]);
					}
					if($this->get('tag.attr_nested')) {
						$this->set('keep.attr_nested', [
							'index' => $this->get('this.index'),
							'level' => $this->get('this.level')
						]);
					}
			...
			elseif($this->get('credential.type', 'php')) {
				if($this->get('tag.tag') == 'arguments') {
					$this->set('tag.open_tag', '(');
					$this->set('tag.close_tag', ')');
				}
				elseif($this->get('tag.tag') == 'var') {
					$this->set('tag.tag_before', '$');
					$this->set('tag.text', $this->get('tag.attr.name'));
				}
			}
	...
	public function finalize() {
		if($this->isset_and_true('settings.unclosed_tags')) {
			...
			if($this->get('this.unclosed_tags') < 0) {
				$this->fail('You have more opened tags than you have opened.');
			}
		}
		$data = $this->get('this.document');
		$data = $this->nested($data);
		$string = $this->build_document($data);
		if(trim($string)) {
			$this->cache($this->get('view'), $string);
		}
		$this->reset();
		return $string;
	}
	...
	public function build_php($data, $level = 0) {
		
	}

	public function build_html($data, $level = 0) {
		
	}

	public function build_document($data, $level = 0, $fix_case = '') {

			$contents = '';

			foreach($data as $key => $item) {

				if($item['credential'] == 'html') {
					// html BUILDER
					if(isset($item['items'])) {
					...
					$item['attr_string'] = "";
					$item['new_line_before'] = $this->new_line();
					$item['new_line_after'] = $this->new_line();
				...
				elseif($item['credential'] == 'php') {
					...
					$contents.=$this->implode('', $item['append_before_tag']);
					$contents.=$item['tag_before'];
					$contents.=$item['open_tag'];
					$contents.=$this->implode('', $item['append_before_text']);
					$contents.=$item['text'];
					$contents.=$this->implode('', $item['append_after_text']);
					$contents.=$item['nested'];
					$contents.=$item['close_tag'];
					$contents.=$item['tag_after'];
					$contents.=$this->implode('', $item['append_after_tag']);
				}
			}
	...
	public function item_defaults($item) {
			return [
			    'credential'				=> $this->key_exists('credential', $item) ? $item['credential'] : 'html',
			    ...
			    'attr_nested'				=> $this->key_exists('attr_nested', $item) ? $item['attr_nested'] : 0,
	...
	public function component_exists($path) {
		return $this->file_exists($this->assets_path($path));
	}

	public function component_make($path) {
		return $this->makeFileForce($this->assets_path($path));
	}
	...
	public function style_to_file($style, $class) {
	    	...
	    	$contents = '';
	    	$file_name = 'views/'.$this->get('view').'/hand/hand.css';
			if(!$this->component_exists($file_name)) {
				$this->component_make($file_name);
				...
				if(!$this->contains_in($style, '{') && !$this->contains_in($style, '}')) {
					...
					foreach($style as $key => $value) {
						if($value !== '') {
							if($contents) {
								$contents.=$this->space(1).$this->new_line();
							}
							$contents.=$value.';';
						}
					}
					...
					if(!$this->contains_in($this->getFileContents($this->assets_path($file_name)), $contents)) {
						$this->file_append($this->assets_path($file_name), $contents);
					}
				}
				return $contents;
			}
	...
	public function require($file, $position = 'require') {
			if($this->file_exists($this->assets_path($file))) {
				...
				if(!$this->isset('settings.'.$ext.'_'.$position)) {
	...
	public function loadTheScriptsComponents() {
			if(!$this->isset('settings.scripts')) {
	...
	public function gg_symbol_string($output) {
	...
	public function gg_alpha_string($output) {
			$output->string.=$output->a_o;
			return $output;
		}

	public function gg_number_string($output) {
			$output->string.=$output->a_o;
			return $output;
		}

	public function gg_control_string($output) {
			$output->string.=$output->a_o;
			return $output;
		}

	public function ident_exists($output, $key) {
			return isset($output->ident[$key]['default']);
		}

	public function gg_symbol($output) {
			if($output->a == '"' || $output->a == "'") {
				$output->isString = 1;
				...
				if($output->a == '_') {
					...
					if($output->tmp_ident !== '') {
						$output->contents.= $output->tmp_ident;
						$output->tmp_ident = '';
					}
					if($output->hstring !== '') {
						$output->hstring = 0;
						$output->contents.=$output->lstring;
					}
				}
			}
			return $output;
	...
	public function gg_alpha($output) {
			$output->tmp_ident.=$output->a;
			$output->tmp_ident_o.=$output->a_o;
			return $output;
		}

	public function gg_number($output) {
			$output->tmp_ident.=$output->a;
			$output->tmp_ident_o.=$output->a_o;
			return $output;
		}

	public function gg_control($output) {
			if($output->tmp_ident !== '') {
				$output->tmp_ident = '';
			}
			return $output;
	...
	public function character($output) {
	...
	public function get_exe_options($input) {
			$output = new stdClass;
			$output->tmp											= '';
			$output->is												= [
				'comment_line' => 0,
	...
	public function results($output) {
			if($output->input) {
	...
	public function log($msg) {
			$logs = $this->mytime()." : ".$msg."\n";
			$logs.= $this->getFileContents($this->disk('system/logs/log.txt'));
			$this->write_file($this->disk('system/logs/log.txt'), $logs);
		}
	...
	public function array_zero($array) {
	...
	public function getFileContents($file, $lock = false) {
    ...
    public function find($options, $something = []) {
    	...
    	if($this->key_exists('i', $options)) {
	    	...
	    	if($options['i'] == $options['count']) {
	    		...
	    		if($options['method'] == 'isset_and_true') {
					return ($something !== 0 && !$this->is_empty($something) && $something !== '' && $something) ? true : false;
				}
				...
				if($this->key_exists($options['array_captions'][$options['i']], $something)) {
		    		...
		    		if($options['method'] == 'isset' || $options['method'] == 'isset_and_true') {
						return 0;
					}
		    		elseif($options['method'] == 'get') {
						$this->fail('Undefined index : [ '.$options['array_captions'][$options['i']].' ] ');
					}
		    		elseif($options['method'] == 'set') {
						return 0;
					}
		    		...
		    		elseif($options['method'] == 'unset') {
						$this->fail('Undefined index for unset : [ '.$options['array_captions'][$options['i']].' ] ');
					} else {
	    	...
	    	$options['caption_key'] = strtolower($options['key']);
	    	$options['array_captions'] = explode('.', $options['caption_key']);
			$options['table'] = $options['array_captions'][0];
			unset($options['array_captions'][0]);
			$options['array_captions'] = array_values($options['array_captions']);
			$options['count'] = count($options['array_captions']);
	...
	public function merge($defaults, $replaces) {
		...
		foreach($replaces as $key => $value) {
			if(!isset($defaults[$key]) || (isset($defaults[$key]) && !is_array($defaults[$key]))) {
				$defaults[$key] = [];
			}
			if(is_array($value)) {
    ...
    public function isset_and_true($key) {
		return $this->find([
			'key' => $key,
			'method' => 'isset_and_true'
		]);
	}
	...
	public function normalize($options) {
		if($options['method'] == 'set') {
			if(!$this->key_exists($options['table'], self::$memory)) {
				self::$memory[$options['table']] = [];
			}
			if($options['count'] > 0) {
				...
				self::$memory[$options['table']] = $this->merge(self::$memory[$options['table']], $data);
			} else {
				self::$memory[$options['table']] = $options['value'];
			}
		...
		elseif($options['method'] == 'isset') {
			if($this->key_exists($options['table'], self::$memory)) {
				if($options['count'] > 0) {
					$options['results'] = $this->find($options, self::$memory[$options['table']]);
				} else {
		    		$options['results'] = self::$memory[$options['table']];
				}
			} else {
				return 0;
			}
		...
		elseif($options['method'] == 'isset_and_true') {
			if($this->key_exists($options['table'], self::$memory)) {
				if($options['count'] > 0) {
					$options['results'] = $this->find($options, self::$memory[$options['table']]);
				} else {
		...
		elseif($options['method'] == 'unset') {
			if($this->key_exists($options['table'], self::$memory)) {
				if($options['count'] > 0) {
					if($options['count'] == 1) {
						unset(self::$memory[$options['table']][$options['array_captions'][0]]);
					} else {

						$tmp = $options;
						$tmp['count'] = $tmp['count'] - 2;
						unset($tmp['array_captions'][$options['count'] - 1]);
						unset($tmp['array_captions'][$options['count'] - 2]);

						$implode = $this->implode('.', $tmp['array_captions']);

						$data = $this->get($tmp['table'].($implode ? '.'.$implode : ''));

						unset($data[$options['array_captions'][$options['count'] - 2]][$options['array_captions'][$options['count'] - 1]]);
						...
						$this->set($tmp['table'].($implode ? '.'.$implode : ''), 0);
						...
						$this->set($tmp['table'].($implode ? '.'.$implode : ''), $data);
					}
				} else {
		    		unset(self::$memory[$options['table']]);
				}
			} else {
				$this->fail('Undefined index for unset : [ '.$options['table'].' ] ');
			}
		}
		elseif($options['method'] == 'get') {
			if($this->key_exists($options['table'], self::$memory)) {
		...
		elseif($options['method'] == 'push') {
			if($options['count'] > 0) {
				$data = $this->find($options, self::$memory[$options['table']]);
				$data[] = $options['value'];
				$options['value'] = $data;
				$options['array_captions'][$options['count']] = $options['value'];
				$data = $this->array_single_to_multidimentional($options['array_captions'], $options['count'] + 1);
	...
	public function getParsedApplication() {
		$project = $this->getFileContents($this->disk('vendor\brosta\interframework\src\Interframework\signal.php'));
		$parser = (new ParserFactory)->create(ParserFactory::PREFER_PHP7);
		$project = $parser->parse($project);
		//$project = json_decode(json_encode($project), true);
		$prettyPrinter = new PrettyPrinter\Standard($project);
		$project = $prettyPrinter->prettyPrintFile($project);
		return $this->is_empty($project) ? '' : $project;
	}
