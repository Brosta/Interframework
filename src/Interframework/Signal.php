...
class Signal {
	...
	public function credential() {
			if(!$this->isset('credential.doctype')) {
				$this->set('credential.doctype', 'html');
			}

			if($this->get('credential.doctype', 'html')) {
			...
			elseif($this->get('credential.doctype', 'php')) {
				...
				if(!$this->is_empty($item['attr'])) {
					foreach($item['attr'] as $attr_name => $attr_value) {
						...
						if($this->is_array($attr_value) || $this->is_object($attr_value)) {
							$attr_value = '';
							$item['attr_string'].=$attr_name.'="hty"';
						} else {
	...
	public function exe($contents) {
			...
			$this->set('credential.doctype', 'php');
	...
	public function __set($key, $value) {echo $value; exit;
	...
	public function __call($method, $arguments) {
		if(!$this->isset('functions.'.$method)) {
			$this->fail('Method [<strong> '.$method.' </strong>] does not exists.');
		} else {
			//echo "\n".$method;
			return call_user_func_array($this->get('functions.'.$method), $arguments);
		}
	}
